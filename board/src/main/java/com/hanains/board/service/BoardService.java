...
import com.hanains.board.dao.BoardDao;
import com.hanains.board.util.Common;
import com.hanains.board.vo.BoardInfo;
...
public class BoardService {
	...
	public Model list(Model model, Long div) {

		List<BoardInfo> list = dao.getListByFaging(1,
				Common.SHOW_BOARD_WRITHING_COUNT_ON_PAGE, div);
		int tempSize = dao.getBoardCount(div);
	...
	public void insert(BoardVo vo, MultipartFile multipartFile[]) {
		...
		if (vo.getGroupNo() == 0 ) {
			long maxValue = 0;
			int boardCount = dao.getBoardCount(vo.getDivision());
			if (boardCount == 0) {
				maxValue = 0;
			} else {
	...
	public Model viewPaging(int index, String keyword, Model model, Long div) {
